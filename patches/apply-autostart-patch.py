...
async def _autostart_session(
    ...
    # every session here. Rename the tmux window (and rebind) so watercooler's
    # @mentions have a unique, addressable handle — but never touch the
    # Telegram topic title itself: Sean's rule is that a topic's title is
    # whatever he typed when he created it, forever. (See
    # apply-no-topic-rename-patch.py, which also neuters the ongoing
    # emoji-status rename path.)
